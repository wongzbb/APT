...
import core.adazoo.schrodinger as schrodinger
...
def setup_schrodinger(model, cfg, logger):
    """Set up TEA adaptation.
    """
    model = configure_model(model, 
                            ada_param=cfg.MODEL.ADA_PARAM)
    params, param_names = collect_params(model, 
                                         ...
                                         logger=logger)
    optimizer = setup_optimizer(params, cfg, logger)
    schrodinger_model = schrodinger.Schrodinger(model, optimizer,
                           steps=cfg.OPTIM.STEPS,
                           threshold = cfg.SCHRODINGER.THRESHOLD, 
                           # alpha = cfg.PL.ALPHA,
                           lr=cfg.OPTIM.LR,
                           epsilon=cfg.SCHRODINGER.EPSILON,
                           alpha=cfg.SCHRODINGER.ALPHA,
                           beta=cfg.SCHRODINGER.BETA,
                           gamma=cfg.SCHRODINGER.GAMMA,
                           episodic=cfg.MODEL.EPISODIC,
                           n_classes=cfg.CORRUPTION.NUM_CLASSES,
                           im_sz=cfg.CORRUPTION.IMG_SIZE, 
                           n_ch = cfg.CORRUPTION.NUM_CHANNEL,
                           ...
                           m = cfg.SCHRODINGER.M,
                           E = cfg.SCHRODINGER.E,
                           V0 = cfg.SCHRODINGER.V0,
                           head_lr = cfg.SCHRODINGER.HEAD_LR,
                           boundary_left = cfg.SCHRODINGER.BOUNDARY_LEFT,
                           boundary_right = cfg.SCHRODINGER.BOUNDARY_RIGHT,
                           delta = cfg.SCHRODINGER.DELTA,
                           InD_threshold = cfg.SCHRODINGER.IND_THRESHOLD,
                           use_schrodinger_loss = cfg.SCHRODINGER.USE_SCHRODINGERHEAd_LOSS,
                           zeta = cfg.SCHRODINGER.ZETA,
                        #    load_head_path = cfg.SCHRODINGER.LOAD_HEAD_DIR,
                           logger = logger)
    logger.info(f"model for adaptation: %s", model)
    logger.info(f"params for adaptation: %s", param_names)
    logger.info(f"optimizer for adaptation: %s", optimizer)
    return schrodinger_model