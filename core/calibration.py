...
def calibration_error(
    logits: torch.Tensor,  # 模型输出的 logits（支持多分类）
    labels: torch.Tensor,  # 真实标签（整数形式，如 [0, 2, 1]）
    n_bins: int = 10,      # 分桶数量
    threshold: float = 1e-6  # 避免空桶的阈值
) -> float:
    # 将 logits 转换为概率（多分类场景）
    if logits.shape != labels.shape:
        probs = torch.nn.functional.softmax(logits, dim=1)
        pred_probs, pred_labels = torch.max(probs, dim=1)
    else:
        # 二分类或已为概率的情况
        pred_probs = logits.sigmoid() if logits.dim() > 0 else logits
    ...
    for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
        ...
        # 计算桶内样本的准确率和平均预测概率
        prob_in_bin = pred_probs[in_bin]
        acc_in_bin = (pred_labels[in_bin] == labels[in_bin]).float().mean()
        avg_prob_in_bin = prob_in_bin.mean()

        # 计算桶的权重（样本数占比）
        weight = in_bin.float().sum() / pred_probs.numel()

        # 累加误差
        ece += weight * torch.abs(acc_in_bin - avg_prob_in_bin)
